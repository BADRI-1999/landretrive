...
# Test with actual values from the curl request shown earlier
district_code = "4"  # Maharashtra
mandal_code = "7"    # Some mandal
village_csv = "village.csv"
def get_villages(district_code, mandal_code):
    ...
    print(f"Fetching villages for district={district_code}, mandal={mandal_code}...")
    print()
    ...
    try:
        ...
        # Save response to file for inspection (workspace path)
        with open("village_response_7_9.html", "w", encoding="utf-8") as f:
            f.write(resp.text)
        print("Response saved to village_response.html")
        print()
        options = []
        # Try to find option tags
        pattern = r'<option\s+value="([^"]*)">([^<]*)</option>'
        matches = re.findall(pattern, resp.text)
        print(f"Found {len(matches)} option tags")
        ...
        # Also try to find the select element itself
        if "ddlGat" in resp.text:
            print("\nFound 'ddlGat' in response")
        if "ddlSelectSearchType" in resp.text:
            print("Found 'ddlSelectSearchType' in response")
        if "<select" in resp.text:
            print(f"Found {resp.text.count('<select')} select tags")
...
get_villages(district_code="7", mandal_code="9")